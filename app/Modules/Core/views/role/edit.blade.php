...
@section('content')
    <div class="page-header"><h1 class="page-title">Sửa Vai trò: {{ $role->name }}</h1></div>
    <div class="card"><div class="card-body">
        <form method="POST" action="{{ route('admin.roles.update', $role->id) }}">
            @csrf @method('PUT')
            <div class="form-group"><label class="form-label">Tên vai trò *</label><input type="text" name="name" class="form-control" value="{{ $role->name }}" required></div>
            <div class="form-group"><label class="form-label">Mô tả</label><input type="text" name="description" class="form-control" value="{{ $role->description }}"></div>
                ...
                <div id="permList" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;">
                    @foreach($permissions as $perm)
                        <label class="perm-label" style="display:flex;align-items:center;gap:6px;font-size:13px;color:var(--text-secondary);cursor:pointer;padding:4px 0;">
                            <input type="checkbox" class="perm-cb" name="permissions[]" value="{{ $perm->id }}" {{ in_array($perm->id, $rolePermissions) ? 'checked' : '' }}> {{ $perm->display_name ?? $perm->name }}
                        </label>
                    @endforeach
                </div>
            </div>
