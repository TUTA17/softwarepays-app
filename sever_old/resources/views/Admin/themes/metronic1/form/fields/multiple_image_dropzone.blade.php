...
<div class="dropzone dropzone-default dropzone-brand" id="{{$field['name']}}_dropzone">
    <div class="dropzone-msg dz-message needsclick">
        <h3 class="dropzone-msg-title"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">{{""}}.</font></font></h3>
        ...
        <span class="dropzone-msg-desc"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">{{""}}</font></font></span>
    </div>
    @foreach($images as $key=>$value)
        @if($value != '')
            <div class="dz-preview dz-compele dz-compele-{{$key}}" data-key="{{$key}}">
                <div class="dz-image"
                     data-background="{{asset($value)}}"
                     style="background-size: cover !important;background: url({{asset($value, 120, 120)}})">
                    <span class="tooltiptext bg-success">{{""}}</span>
                </div>
                <a class="dz-remove text-info" title="Bấm vào để xóa ảnh !" onclick="deleteImg($(this))"
                   data-action="input-compele-{{$key}}">{{""}}</a>
            </div>
        @endif
    @endforeach
</div>
        ...
        $('body').on('change', "#{{$field['name']}}_file", function () {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (limit) {
                        toastr.error('Chỉ cho phép upload '+limit+' ảnh');
                    }
                }
            });
